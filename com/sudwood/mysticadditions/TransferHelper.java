...
public class TransferHelper 
{
	 ...
	 * @return
	 */
	public static int[] getFirstStackThatFits(int x, int y, int z, ItemStack[] inventory, World worldObj)
	{
			...
			for(int i = 0; i < ((IInventory)tile).getSizeInventory(); i++)
			{
				if(((IInventory)tile).getStackInSlot(i) != null)
				{
					for(int ix = 0; ix < inventory.length; ix++)
					{
						...
						{
							if(((IInventory)tile).getStackInSlot(i).stackSize + inventory[ix].stackSize < 64)
							{
								slot[0] = i;
								slot[1] = 1;
								slot[2] = ix;
							...
							else if(((IInventory)tile).getStackInSlot(i).stackSize + inventory[ix].stackSize > 64)
							{
								slot[0] = i;
								slot[1] = 1;
								slot[2] = ix;
    	...
    	int[] space = new int[3];
            ...
            if(tempTile!=null&&tempTile instanceof ISidedInventory && sideAttached != -1)
            {
            	...
            	for(int i = 0; i < slots.length; i++)
            	{
            		...
            		if(((ISidedInventory) tempTile).isItemValidForSlot(slots[i], stack) && ((ISidedInventory) tempTile).getStackInSlot(slots[i]).itemID == stack.itemID  && ((ISidedInventory) tempTile).getStackInSlot(slots[i]).getItemDamage() == stack.getItemDamage())
            		{
            ...
            if(tempTile!=null&&tempTile instanceof IInventory)
            {
            	for(int i = 0; i < ((IInventory) tempTile).getSizeInventory(); i++)
            	{
            		...
            		if(((IInventory) tempTile).isItemValidForSlot(i, stack) &&((IInventory) tempTile).getStackInSlot(i).itemID == stack.itemID &&((IInventory) tempTile).getStackInSlot(i).getItemDamage() == stack.getItemDamage())
            		{
